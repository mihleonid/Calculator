...
$fileerrors=true;
echo("<p style=\"color: #ff7777\">Нет русского расширения.<a target=\"_blank\" href=\"http://localhost/calculator/rus.zip\">Скачать</a>.</p><p style=\"color: #88bb88\">Заменено английским.</p>");
$file=fopen("empty.file", "a+");
fclose($file);
}
if(!file_exists("favicon.ico")){
$fileerrors=true;
echo("<p style=\"color: #ff7777\">Нет файла иконки.<a target=\"_blank\" href=\"http://localhost/calculator/favicon.zip\">Скачать</a>.</p>");
fclose($file);
}
if((!file_exists("OpenSans-CondLight.ttf")) or (!file_exists("Pangolin-Regular.ttf")) or (!file_exists("Play-Regular.ttf"))){
$fileerrors=true;
echo("<p style=\"color: #ff7777\">Нет шрифтов.<a target=\"_blank\" href=\"http://localhost/calculator/font.zip\">Скачать</a>.</p>");
$file=fopen("empty.file", "a+");
fclose($file);
}
if((!file_exists("bg1.png")) or (!file_exists("bg7.png")) or (!file_exists("bg5.png")) or (!file_exists("bg2.png") or (!file_exists("bg0.png")))){
$fileerrors=true;
echo("<p style=\"color: #ff7777\">Нет файлов настройки фона.<a target=\"_blank\" href=\"http://localhost/calculator/bgs.zip\">Скачать</a>.</p>");
$file=fopen("empty.file", "a+");
fclose($file);
}
...
fclose($ll);
if($llll>60){
	echo("Лог не пустой. <a href=\"viewerr.php\" target=\"_blank\">Просмотреть его</a>.");
}
$file=fopen("nav.html", "r");
...
echo(htmlentities(fread($file, filesize($_POST['openo'])), 0, 'utf-8'));
fclose($file);
echo("</TEXTAREA>
...
Широкий экран<br>
<iframe class=\"ame\" src=\"index.php\"></iframe><br>
Узкий экран<br>
<iframe style=\"height: 300px; width: 400px;\" src=\"index.php\"></iframe></div>");
}else{
sleep(5);
